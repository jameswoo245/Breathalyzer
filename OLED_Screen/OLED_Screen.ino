...
void loop() {
  // Check if it's time to read the mq-3, ultrasonic, and temperature sensors
  if (millis() - lastSensorReadTime >= sensorInterval) {
    lastSensorReadTime = millis();
    float bac = readAlc() // NEW CHANGE
    int distance = readUltrasonicSensor();
    float temperatureF = readTemperatureSensor();
    sendDataToServer(bac, distance, temperatureF); // NEW CHANGE, ADDED BAC
  }
...
void sendDataToServer(float bac, int distance, float temperatureF) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Create JSON payload
    String payload = "{";
    payload += "\"bac\":" + String(bac, 2) + ","; // NEW CHANGE
